...
public class Agent extends AbstractPlayer {
    ...
    private static int SIMULATION_DEPTH = 40; //每个 episode 的长度，即模拟采样时向前模拟步数的长
    ...
    //是存储状态的最大个数，即估算 Q 值的决策树模型中最多的状态数，因为 pacman 的状
    // 态数量太多无法一一存储，所以需要用决策树存储少量状态来估算其余未知状态的 Q 值。
    protected int m_maxPoolSize = 2000;
    //折扣因子，为了平衡现在和未来的 reward，使 reward 收敛，折扣因子越大表示对未来的回
    // 报越感兴趣，折扣因子为 0 时程序是短视的；
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        learnPolicy(stateObs, SIMULATION_DEPTH, new SimpleStateHeuristic(stateObs));//new WinScoreHeuristic(stateObs));
    ...
    private Instances simulate(StateObservation stateObs, StateHeuristic heuristic, QPolicy policy) {
        ...
        int depth = 0;
        double factor = 1;
        for (; depth < SIMULATION_DEPTH; depth++) {//最大向下探索20步
            try {
                double[] features = RLDataExtractor.featureExtract(stateObs);
